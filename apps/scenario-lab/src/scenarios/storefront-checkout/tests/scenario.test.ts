...
const CARD_FRAME = "frame:Secure card payment/";

/** The opening tag of the form control a `testid:` target names, in the document it lives in; undefined for anything else. */
function formControlTag(target: string): string | undefined {
  const inFrame = target.startsWith(CARD_FRAME);
  const id = (inFrame ? target.slice(CARD_FRAME.length) : target).slice("testid:".length);
  const documents = inFrame ? [cardFramePage()] : [startPage(), render(completedCheckout("approve"), context)];
  return documents
    .flatMap(html => [...html.matchAll(/<(?:input|textarea|select)\b[^>]*>/gu)].map(match => match[0]))
    .find(tag => tag.includes(` data-testid="${id}"`));
}
...
test("a secret is declared for exactly the steps that type into a marked control, each one pairable by its test id", () => {
  ...
  const marked = steps.filter(step => {
    ...
    if (step.operation === "type") assert.ok(tag, `${step.id} types into a form control the documents render`);
  ...
  assert.deepEqual(marked.map(step => step.id), ["enter-account-password", "enter-card-number", "enter-card-name", "enter-card-expiry", "enter-billing-card-number"]);
  ...
  // The Flow lane pairs a `testid:` target with the control's recorded test id,
  // so each target is one, and no two name the same control.
  const targets = secrets.map(secret => steps.find(step => step.id === secret.step)?.target ?? "");
  for (const target of targets) assert.match(target, /^(?:frame:[^/]+\/)?testid:[a-z0-9-]+$/u);
  assert.equal(new Set(targets.map(target => target.slice(target.indexOf("testid:")))).size, targets.length);
  ...
  // The security code is undeclared because its field carries no marking.
  const securityCode = formControlTag(`${CARD_FRAME}testid:card-security-code`);
  assert.ok(securityCode);
  assert.equal(isMarkedSensitive(securityCode), false);
});
