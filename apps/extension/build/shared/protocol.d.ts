...
export declare const CLIENT_GATEWAY_PROTOCOL_VERSION = "0.1";
...
export type RecordingEventKind = "content.ready" | "browser.tab" | "browser.navigation" | "dom.click" | "dom.input" | "dom.change" | "dom.submit" | "dom.focus" | "dom.blur" | "dom.keydown" | "dom.scroll" | "dom.mutation" | "dom.snapshot" | "action.result" | "client.error";
...
export type BrowserActionType = "browser.navigate" | "dom.click" | "dom.type" | "dom.clear" | "dom.select" | "dom.scroll" | "dom.keypress" | "dom.wait_for_selector" | "dom.wait_for_text" | "dom.extract" | "dom.capture_snapshot";
...
export type ClientMessage = ProtocolEnvelope<"client.hello", ClientHelloPayload> | ProtocolEnvelope<"client.capabilities", {
    capabilities: ClientGatewayCapability[];
}> | ProtocolEnvelope<"client.browser_state", ClientGatewayBrowserState> | ProtocolEnvelope<"client.tab_state", JsonObject> | ProtocolEnvelope<"client.recording_event", ClientGatewayRecordingEvent> | ProtocolEnvelope<"client.dom_snapshot", ClientGatewaySnapshot> | ProtocolEnvelope<"client.action_result", ClientGatewayActionResult> | ProtocolEnvelope<"client.error", {
    message: string;
    code?: string | undefined;
    metadata?: JsonObject | undefined;
}>;
export type ServerMessage = ProtocolEnvelope<"server.pairing_required", ServerPairingRequiredPayload> | ProtocolEnvelope<"server.session_ready", ServerSessionReadyPayload> | ProtocolEnvelope<"server.start_recording", {
    recordingId: string;
    projectId?: string | null | undefined;
    taskId?: string | undefined;
}> | ProtocolEnvelope<"server.stop_recording", {
    recordingId?: string | undefined;
}> | ProtocolEnvelope<"server.capture_snapshot", {
    kind?: string | undefined;
    metadata?: JsonObject | undefined;
}> | ProtocolEnvelope<"server.execute_action", ClientGatewayActionCommand> | ProtocolEnvelope<"server.set_active_tab", {
    tabId: string;
}> | ProtocolEnvelope<"server.ping", {
    nonce: string;
}> | ProtocolEnvelope<"server.disconnect", {
    reason: string;
}> | ProtocolEnvelope<"server.error", {
    message: string;
    code?: string | undefined;
    metadata?: JsonObject | undefined;
}>;
export declare const browserExtensionCapabilities: ClientGatewayCapability[];
export declare function createClientEnvelope<TType extends ClientMessage["type"], TPayload extends object>(params: {
    type: TType;
    clientId: string;
    sessionId?: string | undefined;
    correlationId?: string | undefined;
