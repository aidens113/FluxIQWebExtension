...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    ...
    if (typed.type === "executeAction" && typed.action) {
        void executeAction(typed.action)
            .then(sendResponse)
            .catch((error) => sendResponse(actionFailure(typed.action, error)));
        return true;
    }
    return false;
});
...
function emit(kind, details) {
    if (!recording && kind !== "content.ready")
        return;
...
function basePayload(kind, details) {
    ...
    if (details.element)
        payload.element = details.element;
    if (details.snapshot && captureSnapshots)
        payload.snapshot = details.snapshot;
    if (details.inputValue !== undefined)
        payload.inputValue = details.inputValue;
    if (details.key !== undefined)
        payload.key = details.key;
    if (details.scroll)
        payload.scroll = details.scroll;
    if (details.mutation)
        payload.mutation = details.mutation;
    if (details.actionResult)
        payload.actionResult = details.actionResult;
    if (details.metadata)
...
function resolveTarget(action) {
    if (action.selector) {
        ...
        if (!element)
            throw new Error(`No element matches selector: ${action.selector}`);
        return element;
    }
    if (action.coordinates) {
        const element = document.elementFromPoint(action.coordinates.x, action.coordinates.y);
        if (!element)
            throw new Error("No element exists at the requested coordinates.");
        return element;
    }
...
function captureSnapshot() {
    ...
    if (selectedText)
        snapshot.selectedText = selectedText.slice(0, 2_000);
    return snapshot;
}
function describeElement(element) {
    ...
    if (value !== undefined && captureInputValues)
    ...
    if (element instanceof HTMLAnchorElement && element.href)
        descriptor.href = element.href;
    if (element instanceof HTMLInputElement && element.type)
        descriptor.inputType = element.type;
    const attributes = {};
...
function readElementValue(element) {
    ...
    if (element instanceof HTMLElement && element.isContentEditable)
        return element.innerText.slice(0, 2_000);
    return undefined;
}
...
function waitForText(text, timeoutMs = 10_000) {
...
function extractElement(element, options) {
    ...
    if (mode === "attribute" && typeof options?.attribute === "string")
