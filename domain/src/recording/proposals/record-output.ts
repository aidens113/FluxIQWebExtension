...
//
// Core's `recordOutput` on a mapper candidate is what turns a recorded
// extraction into a node that stores rows: the proposal lift parses it, writes
// it into the approved Flow node's `parameterValues.recordOutput`, and the
// executor captures the node's records against the schema it declares (Core K3,
// K4a, K7). An invalid one rejects the whole candidate rather than being
// dropped, so everything built here has to satisfy Core's parser.
//
...
//
// There is no `recordsPath`. Core takes it from the output's own
// `metadata.recordsPath` (`output-nodes/definitions.ts`, CD19), so naming it
// here would be a second place for the key to drift.
...
/**
 * The dataset a recorded list extraction saves into: its id and name, a schema
 * over every field the request declares, and the rows one run may keep.
 *
 * `writeMode` is `append`, so a paginated read's later pages add to the rows the
 * earlier ones stored rather than replacing them.
 */
export function webAutomationRecordOutput(definition: WebAutomationRecordedListExtraction): WebAutomationRecordOutput {
