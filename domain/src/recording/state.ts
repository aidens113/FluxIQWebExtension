...
export function createWebAutomationInitialState(timestamp = Date.now()): StateSnapshot {
  return {
    timestamp,
    namespaces: {
...
export function withWebStateValue(snapshot: StateSnapshot, path: string, value: unknown, input: { observedAt?: number; sourceId?: string; metadata?: JsonObject } = {}): StateSnapshot {
...
function inferStateType(value: unknown): StateValue["type"] {
