// Recording state projection: the initial state and the DOM snapshot -> state
// snapshot mapping, with its element filtering and visual frames. Moved
// verbatim out of domain/src/tests/domain.test.ts.
...
import { createWebAutomationStateFromSnapshot, filterStateElements, webAutomationActionVisualTargetFromElement } from "../web-state";
...
const saveScreenLayer = snapshotState.presentation?.visualFrames?.[0]?.layers.find((layer) => layer.id.includes("button.save"));
assert.equal(saveScreenLayer !== undefined && "statePath" in saveScreenLayer ? saveScreenLayer.statePath : undefined, "web.elements.button.save");
...
assert.equal((iframeScreenFrame?.metadata?.frameViewportOffset as { x?: number } | undefined)?.x, 900);
