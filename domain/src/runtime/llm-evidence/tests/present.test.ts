...
const dialog = (): WebLlmEvidenceDialog =>
  present<WebLlmEvidenceDialog>({ role: "dialog", name: undefined, modal: true, selector: "#confirm" });
...
test("false, 0 and the empty string are values and survive", () => {
  const element = present<WebLlmEvidenceElement>({
    target: "target.1", tag: "input", selector: "#a", frameId: 0, role: "", name: undefined, text: undefined,
...
test("key order is the literal's, so the serialized bytes are the writer's own", () => {
  const value = present<WebLlmEvidenceDialog>({ selector: "#s", modal: undefined, name: "Name", role: "dialog" });
...
test("the compile-time rows above and below are the real assertions", () => {
  ...
  present({ role: "dialog", name: undefined, modal: undefined, selector: undefined });

  // @ts-expect-error 'roel' is not a field of WebLlmEvidenceDialog
  present<WebLlmEvidenceDialog>({ roel: "dialog", name: undefined, modal: undefined, selector: undefined });

  // @ts-expect-error 'selector' was deleted, which is how an optional field silently leaves the packet
  present<WebLlmEvidenceDialog>({ role: "dialog", name: undefined, modal: undefined });
  ...
  present<WebLlmEvidenceElement>({
    ...
    target: undefined,
    tag: "input", selector: "#a", frameId: undefined, role: undefined, name: undefined,
  ...
  assert.deepEqual(present<WebLlmEvidenceDialog>({ role: undefined, name: undefined, modal: undefined, selector: undefined }), {});
});
