    ...
    tools: [
      {
        ...
        description: "Detect the repeating list or table an extraction would read: around an observed element when given its opaque target handle, else the page's largest list. Returns an opaque extraction handle naming it, each field's key, label, kind and coverage, the item count, and how the list continues. Returns no values or selectors. Observes only, and is never a step of the Flow. Write the list into the extraction node as extractList: {handle, fields?: {yourKey: \"fieldKey\" | \"fieldKey@attr\"}, where?: [{field: \"fieldKey\", is: \"absent\"}], paginate?: false, minItems?: 0}, then run that node to see the rows it really reads. Its count is the whole list: where the Flow returns only part of it, narrow the page first, minItems: 0 where the answer may be no rows. where says which items are rows at all, and every condition must hold: {field, is: \"present\"} or {field, is: \"absent\"} for a column only some items have, and atLeast, atMost, lessThan or greaterThan for the number in a column (an item whose column is missing or holds no number fails a bound). A detected list mixes advertisements in with results, because the page draws both from one template, so a column with coverage below 1 is often the mark that tells them apart -- leave the advertisements out with is: \"absent\" on it, and run the node to check the rows. There is no test against the text in a column: narrow by a column the page gives some items and not others.",
