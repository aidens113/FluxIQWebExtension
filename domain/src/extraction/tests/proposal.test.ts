// A proposal holds selectors, names and counts only, never a page value (D3).
// The one place a page value could ride in through the contract is a field
// spec's element fingerprint, which records text, value and link target, so the
// proposal's spec leaves it out. The `@ts-expect-error` below is checked by the
// domain type check: widen the spec back to the recorded one and the check
// fails on an unused directive.
//
// A proposal is also what the harness and the picker send back as the request,
// so its fields must be a valid `extract_list` field map as they stand.
...
test("a proposed field's spec cannot carry an element fingerprint", () => {
  const field: WebAutomationExtractionProposalField = {
    key: "product_name",
    label: "product-name",
