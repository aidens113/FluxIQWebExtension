// domain/src/recording/web-state/tests/evidence.test.ts
...
var evidence = {
  ...
  dialogs: {
    ...
    modal: true,
    armPending: true,
    lastNative: { kind: "confirm", message: "Leave this page?", response: "dismiss", at: 1700, promptText: LEAKED }
  },
  overlays: {
    tested: 24,
    blockedCount: 2,
...
function stateFor(pageEvidence, elements = [{ tagName: "button", selector: "button.pay", text: "Pay", bounds: { x: 20, y: 400, width: 120, height: 40 } }]) {
  return createWebAutomationStateFromSnapshot({
    url: "https://example.test/checkout",
    title: "Checkout",
    viewport: { width: 1280, height: 720, scrollX: 0, scrollY: 0 },
    interactiveElements: elements,
    ...pageEvidence === void 0 ? {} : { evidence: pageEvidence }
  }, { timestamp: 40, sourceId: "tab:9" });
}
function valuesOf(state) {
  return state.namespaces.web?.values ?? {};
}
function collectionAt(state, path) {
  return valuesOf(state)[path]?.value;
}
test("every evidence path mirrors the evidence's own field path, under one prefix", () => {
  ...
  assert.equal(values["evidence.navigation.url"], void 0);
...
test("an empty collection is not written at all", () => {
  ...
  assert.equal(values["evidence.regions"], void 0);
  assert.equal(values["evidence.repeating"], void 0);
  assert.equal(values["evidence.forms"], void 0);
...
test("a sensitive control keeps its identity and loses even the presence of its value", () => {
  const controls = collectionAt(stateFor(evidence), "evidence.forms").items[0]?.controls;
  ...
  assert.equal(card?.hasValue, void 0, "presence is the only field here derived from a secret, so it is withheld");
  ...
  assert.equal(email?.sensitive, void 0);
});
test("the sensitivity rule is asked again here, so a password control is protected without the producer's flag", () => {
  ...
  const card = (collectionAt(stateFor(unflagged), "evidence.forms").items[0]?.controls)[0];
  assert.equal(card?.sensitive, true, "the shared rule reads the control type, not the producer's word for it");
  assert.equal(card?.hasValue, void 0);
});
test("no field the evidence shape does not declare reaches state, however it arrives", () => {
  ...
  const native = valuesOf(stateFor(evidence))["evidence.dialogs.lastNative"]?.value;
...
test("evidence that arrives malformed is skipped rather than thrown or written", () => {
  ...
  assert.equal(values["evidence.loading.pendingNavigation"], void 0, "a field the page did not offer is absent, not guessed");
