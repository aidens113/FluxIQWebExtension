...
test("the record output is one Core's own parser accepts", () => {
  const parsed = parseAutomationStudioRecordOutput2({ ...webAutomationRecordOutput(definition()), recordsPath: RECORDS_PATH });
...
test("two columns with the same header do not make a schema Core refuses", () => {
  ...
  const parsed = parseAutomationStudioRecordOutput2({ ...output, recordsPath: RECORDS_PATH });
  assert.equal(parsed.ok, true, parsed.ok ? "" : parsed.issues.join(", "));
});
