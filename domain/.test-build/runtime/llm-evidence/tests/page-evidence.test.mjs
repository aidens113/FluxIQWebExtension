...
test("reports the pre-filter element total the capture declares, and that the capture itself truncated", () => {
  const declared2 = sanitizeWebLlmSnapshot(page({}, { elementTotal: 812, truncated: true }));
  assert.equal(declared2.elementTotal, 812);
  assert.equal(declared2.truncated, true);
  assert.equal(declared2.captureTruncated, true, "the capture cut, so narrowing the capture is the remedy");
  assert.equal(declared2.elementsTruncated, void 0, "the packet's own bound was nowhere near");
  assert.equal(declared2.budgetTruncated, void 0, "and the budget was not what cut");
