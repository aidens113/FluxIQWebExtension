...
function elementFingerprint(value) {
  const element2 = objectValue(value);
  if (!element2) return void 0;
  const attributes = elementAttributes(element2.attributes);
  return compact({
    selector: stringValue(element2.selector),
    xpath: stringValue(element2.xpath),
    id: stringValue(element2.id),
    classNames: Array.isArray(element2.classNames) ? element2.classNames.filter((item) => typeof item === "string") : void 0,
    visibleText: stringValue(element2.visibleText),
    tagName: stringValue(element2.tagName),
    text: stringValue(element2.text),
    value: stringValue(element2.value),
    role: stringValue(element2.role),
    implicitRole: stringValue(element2.implicitRole),
    name: stringValue(element2.name),
    href: stringValue(element2.href),
    inputType: stringValue(element2.inputType),
    checked: booleanValue(element2.checked),
    testId: elementTestId(element2, attributes),
    accessibleName: stringValue(element2.accessibleName) ?? stringValue(attributes?.["aria-label"]),
    label: stringValue(element2.label),
    attributes,
    context: elementContext(element2.context),
...
function elementTestId(element2, attributes) {
  return stringValue(element2.testId) ?? stringValue(attributes?.["data-testid"]) ?? stringValue(attributes?.["data-test"]) ?? stringValue(attributes?.["data-cy"]);
}
...
function fieldValue(value) {
  ...
  const element2 = optionalValue(spec.element, fingerprintValue);
  if (selector === REFUSED || required === REFUSED || handling === REFUSED || element2 === REFUSED) return void 0;
  const field = {
    kind,
    ...
    ...element2 !== void 0 ? { element: element2 } : {}
  };
  return field;
}
...
test("every declared parameter name is a name Core will carry as a handle key, and says what its handle is", () => {
  ...
  for (const definitionId of definitionIds) {
    ...
    assert.ok(declared2.length > 0, definitionId);
