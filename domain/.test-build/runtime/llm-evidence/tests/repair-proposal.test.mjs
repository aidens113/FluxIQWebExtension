...
function isRepairResolution(target2) {
  return objectValue(target2.handles) !== void 0 && (target2.handleResolution === "named" || target2.handleResolution === "inferred");
}
...
function recordedControl(recordedTarget) {
  ...
  const target2 = objectValue(recordedTarget.target);
  const adapted = [target2?.element, target2?.fingerprint, recordedTarget.target];
...
var createdNode = (id, definitionId) => ({
  id,
  definitionId,
...
var TARGET_NOT_FOUND = { category: "target_not_found", code: "web.target.not_found", retryable: true };
var proposeThroughCore = (binding, node, proposed, failure = TARGET_NOT_FOUND) => proposeAutomationStudioRuntimeTargetOverride({
...
test("Core keeps no proposal for a failure a different target cannot fix, and never asks the domain", () => {
  for (const category of ["navigation_unexpected", "blocked_by_capability_or_policy", "auth_required"]) {
    ...
    assert.equal(result.adaptation, void 0, category);
    assert.equal(result.changeProposal, void 0, category);
  }
});
