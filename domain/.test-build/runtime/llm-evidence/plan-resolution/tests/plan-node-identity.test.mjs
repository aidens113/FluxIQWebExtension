...
function recordedControl(recordedTarget) {
  ...
  for (const source of sources) {
    ...
    const controlType = text2(fingerprint.attributes?.type) ?? text2(metadata?.controlType);
    return {
      fingerprint,
      formId: text2(fingerprint.context?.formId) ?? text2(fingerprint.context?.formName) ?? text2(metadata?.formId),
      controlType,
...
function identifiesAControl(fingerprint) {
  return text2(fingerprint.tagName) !== void 0 || text2(fingerprint.role) !== void 0 || recordedNames(fingerprint).length > 0;
}
...
function recordedNames(fingerprint) {
  ...
  return [fingerprint.accessibleName, fingerprint.visibleText, fingerprint.text, fingerprint.label, attributes?.["aria-label"], attributes?.title].flatMap((value) => text2(value) === void 0 ? [] : [value]);
}
function evidenceNames(element) {
  const all = [element.name, element.text].flatMap((value) => text2(value) === void 0 ? [] : [value]);
