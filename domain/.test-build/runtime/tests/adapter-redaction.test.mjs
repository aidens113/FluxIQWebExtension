...
for (const [client, withheld] of withholdingClients) {
  for (const declaration of [void 0, false, true]) {
    test(`the three-state probe, ${client}: declaration ${String(declaration)}`, async () => {
      const declared2 = declaration === true;
      const expected = declared2 ? redactedPhrasing : `the field holds "${producerSentinel}"`;
      ...
      const result = await runCommand({
        ...
        payload: sensitivePayload({ status: "failed", validation: declared2 ? produced : withheld("failed") })
      });
