// domain/src/client/tests/gateway-mapping.test.ts
...
function webAutomationExtractionSummaryValue(value) {
  ...
  const recordCount = countValue(summary2.recordCount);
  const pagesRead = countValue(summary2.pagesRead);
  const fieldNames = fieldKeyList(summary2.fieldNames);
  const missingFields = fieldKeyList(summary2.missingFields);
  if (recordCount === void 0 || pagesRead === void 0 || typeof summary2.truncated !== "boolean" || fieldNames === void 0 || missingFields === void 0) return void 0;
  if (!missingFields.every((key) => fieldNames.includes(key))) return void 0;
  return { recordCount, pagesRead, truncated: summary2.truncated, missingFields, fieldNames };
}
...
var extractionSentinel = "SENTINEL-PAGE-VALUE-FROM-THE-PICKER";
var recordedExtraction = {
  form: "list",
...
var extractionEvent = createWebAutomationRecordingEvent({
  kind: "data.extract",
  sequence: 11,
...
var measured = { strategy: "scored-candidate", candidateCount: 3, bestScore: 0.51, runnerUpScore: 0.28, confidence: 0.51 };
var carried = webAutomationActionResultPayload({ ...failedValidationResult, resolution: measured });
assert.deepEqual(carried.resolution, measured, "the result mapping carries the resolution measurement");
var resolutionStrings = Object.entries(measured).filter(([, value]) => typeof value === "string").map(([key]) => key);
assert.deepEqual(
  resolutionStrings,
  ["strategy"],
...
var encryptSentinel = "SENTINEL-SELECTOR-OF-AN-ENCRYPTED-FIELD";
function extractListCommand(handling, actionType = "web.dom.extract_list") {
...
var encryptedCommand = extractListCommand("encrypt");
assert.equal("status" in encryptedCommand, true, "an encrypt field is not dispatched");
var encryptedFailure = "failure" in encryptedCommand ? encryptedCommand.failure : void 0;
...
assert.equal(
  webAutomationActionFromGatewayCommand({ commandId: "command.both", actionType: "web.dom.extract_list", parameters: { extractList: { item: "tr", fields: { card: { kind: "text", handling: "encrypt" }, price: { kind: "column" } } } } }).failure?.code,
  "web.action.invalid_parameter"
);
var declaredFieldKeys = ["name", "price"];
var summary = { recordCount: 8, pagesRead: 1, truncated: false, missingFields: ["price"], fieldNames: declaredFieldKeys };
...
assert.deepEqual(webAutomationActionResultPayload({ ...extractionResult, extraction: { ...summary, truncated: true, recordCount: 1e3 } }).extraction, { ...summary, truncated: true, recordCount: 1e3 });
function stringsIn(value) {
  if (typeof value === "string") return [value];
  if (Array.isArray(value)) return value.flatMap(stringsIn);
  return value && typeof value === "object" ? Object.values(value).flatMap(stringsIn) : [];
}
var summarySentinel = "SENTINEL-PAGE-TEXT-BESIDE-THE-SUMMARY";
var widenedSummary = webAutomationActionResultPayload({ ...extractionResult, extraction: { ...summary, sample: summarySentinel, pages: [{ title: summarySentinel }] } }).extraction;
...
for (const [why, extraction] of malformedSummaries) {
  assert.equal("extraction" in webAutomationActionResultPayload({ ...extractionResult, extraction }), false, why);
}
...
var observedDialog = { kind: "prompt", message: "Name this report", response: "accept", promptText: "Quarterly", at: 17e11 };
var dialogResult = {
...
var dialogPayload = webAutomationActionResultPayload(dialogResult);
...
assert.deepEqual(
  webAutomationActionResultPayload({ ...dialogResult, dialog: { kind: "confirm", message: "Delete?", response: "dismiss", at: 5, html: "<b>dropped</b>" } }).dialog,
...
for (const dialog of [{ ...observedDialog, kind: "toast" }, { ...observedDialog, response: "ignore" }, { ...observedDialog, at: "now" }, { ...observedDialog, promptText: 7 }, "Name this report"]) {
  assert.equal("dialog" in webAutomationActionResultPayload({ ...dialogResult, dialog }), false, JSON.stringify(dialog));
}
