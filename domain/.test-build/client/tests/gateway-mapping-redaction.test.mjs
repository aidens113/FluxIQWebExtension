...
function observedDialogValue(value) {
  const dialog = jsonObject2(value);
...
function sensitiveResult(validation) {
  return {
    ...
    element: { tagName: "input", selector: '[data-testid="payment"]', inputType: "text", attributes: { autocomplete: "billing cc-number" } },
    startedAt: 100,
    finishedAt: 140
  };
}
...
var absentDeclarations = [
  ["no flag at all, which is every verb nobody has taught it yet", void 0],
...
for (const [what, redacted] of absentDeclarations) {
  test(`an absent declaration withholds: ${what}`, () => {
    ...
    const payload = webAutomationActionResultPayload(sensitiveResult(validation));
...
test("the declaration is not a way to keep a comparison on an ordinary control from being read", () => {
  const ordinary = {
    ...
    element: { tagName: "input", selector: 'input[name="username"]', inputType: "text", attributes: { autocomplete: "username" } }
  };
