// src/client/tests/structure-detection-wire.test.ts
...
function observedDialogValue(value) {
  const dialog = jsonObject3(value);
...
// src/client/tests/structure-detection-wire.test.ts
function snapshotCommand(parameters) {
  const mapped = webAutomationActionFromGatewayCommand({ commandId: "c1", actionType: "web.dom.capture_snapshot", parameters });
...
test("the detection flag is lifted onto the command, and a malformed one is left off rather than coerced", () => {
  ...
  assert.equal(snapshotCommand({}).detectStructure, void 0);
  const malformed = snapshotCommand({ detectStructure: { selector: "" } });
  assert.equal(malformed.detectStructure, void 0);
...
test("the page's detection reaches the gateway payload copied, and a malformed one does not reach it at all", () => {
  ...
  const structure = {
    ...
    proposal: {
      ...
      item: "#list > li",
      itemCount: 3,
      fields: [{ key: "name", label: "name", spec: { kind: "text", selector: ".name" }, coverage: 1 }],
      confidence: 1
    }
  };
  const noisy = { ...structure, sample: "Ada Lovelace", proposal: { ...structure.proposal, sample: "Ada Lovelace" } };
  const payload = webAutomationActionResultPayload({ ...base, structure: noisy });
  ...
  assert.equal("structure" in webAutomationActionResultPayload({ ...base, structure: { ok: false, refused: "because" } }), false);
...
test("the client declares detection as a snapshot capability that makes nothing new executable", () => {
  const declared2 = webAutomationClientCapabilities.filter((capability) => capability.id === WEB_AUTOMATION_STRUCTURE_DETECTION_CAPABILITY_ID);
  assert.equal(declared2.length, 1);
  assert.equal(declared2[0].kind, "snapshot");
  assert.equal(declared2[0].actionTypes, void 0);
  assert.deepEqual(declared2[0].metadata, { domainId: "web-automation", actionType: "web.dom.capture_snapshot", parameter: "detectStructure" });
