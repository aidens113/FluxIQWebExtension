// src/io/tests/manifest-definitions.test.ts
...
function nodeDefinition(outputId) {
...
test("the manifest declares element targeting for exactly the outputs that cannot run without an element", () => {
  const declared2 = WEB_AUTOMATION_ACTION_TYPES.filter((outputId) => manifestOutput(outputId).metadata?.elementTarget === true).sort();
  assert.deepEqual(declared2, [...ELEMENT_TARGETED_OUTPUTS].sort());
});
...
test("an output that is not element targeted declares no element-target metadata at all", () => {
  for (const outputId of WEB_AUTOMATION_ACTION_TYPES) {
    ...
    assert.equal(manifestOutput(outputId).metadata?.elementTarget, void 0, `${outputId} carries no element-target flag`);
    if (RECORD_OUTPUTS[outputId] === void 0) {
      assert.equal(manifestOutput(outputId).metadata, void 0, `${outputId} carries no metadata`);
    }
  }
});
