...
async function execute(outputId, parameters) {
  ...
  const context = { inputs: {}, parameters, log: () => void 0 };
...
test("an implementation emits exactly one dispatch effect naming its own output", async () => {
  const result = await execute("web.dom.click", { selector: "#save", timeoutMs: 1e4 });
  assert.deepEqual(result.effects, [{
    type: "policy.output.dispatch",
    payload: { outputId: "web.dom.click", parameters: { selector: "#save", timeoutMs: 1e4 } }
  }]);
});
