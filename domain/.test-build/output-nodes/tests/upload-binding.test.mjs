// domain/src/output-nodes/tests/upload-binding.test.ts
...
test("the request is a path under its own namespace, with no fallback", () => {
  const request = webAutomationUploadBinding("attachment");
...
test("only a request on the upload namespace is an upload request, and a secret request is not one", () => {
  ...
  for (const value of [
    ...
    assert.equal(webAutomationUploadBindingPath(value), void 0, JSON.stringify(value) ?? "undefined");
  }
});
