...
export type RunEvaluation = {
   ...
   *
   * A failed run that got no repair says why in `refusalCode` and which rung
   * decided in `refusalRung`: the gate, when the Flow's settings refused the
   * model, its training budget was spent, or a deterministic answer must come
   * first; or one of the loop's own stages, when it engaged and then asked for
   * no patch. Without the first, "recovery was not allowed to start" and
   * "recovery was not needed" were the same `attempted: false`; without the
   * second, a recovery that ran, diagnosed and repaired nothing said nothing at
   * all -- the outcome that most needs explaining.
   *
