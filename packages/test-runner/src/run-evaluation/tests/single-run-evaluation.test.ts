...
test("a Flow-lane run reports the persisted Core run, not the recording lane's probe", () => {
  const observation = flowLaneObservation({
    ...
    run: {
      runId: "core-run.1", status: "succeeded", harnessActivations: 2, harnessRecovery: RECOVERED, failure: null, resultVerification: "confirmed", extracted: [], extractedNonStringValues: 0, extractionDurationsByNode: new Map(), route: null,
      actions: [{ actionType: "web.dom.click", nodeId: "node-1", attemptIndex: 0, status: "succeeded", startedAt: "2026-09-12T10:00:11.000Z", durationMs: 210, failure: null }],
    },
  });
...
const createdFlow = flowLaneObservation({
  flowCreated: true, oracleVerdict: "passed", automationFailureExpected: null,
  run: { runId: "core-run.1", status: "succeeded", harnessActivations: 0, harnessRecovery: NO_RECOVERY, failure: null, resultVerification: "confirmed", extracted: [], extractedNonStringValues: 0, extractionDurationsByNode: new Map(), route: null, actions: [{ actionType: "web.dom.click", nodeId: "node-1", attemptIndex: 0, status: "succeeded", startedAt: "2026-09-12T10:00:11.000Z", durationMs: 210, failure: null }] },
});
