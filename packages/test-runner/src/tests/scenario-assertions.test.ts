...
test("the runner checks page facts only from the schedule, and the armed set only once armed", async () => {
  ...
  // The armed check sits after the Flow lane's arm, which only an armed run
  // makes, and the load every Flow run makes, so it sees the rendering the Flow
  // is about to run against and not the recorded one.
  assert.ok(afterRecording < flowLoad && flowLoad < afterArm.index, "the armed check runs after the Flow lane's arm and its load before the run");
});

/**
 * Where the Flow lane's load before a run *goes*, which is a separate defect
 * from when it happens. Once armed it was `page.reload()` -- wherever the
 * recording had left the page -- so a fixture whose recording navigates was
 * armed, reloaded onto its last page, checked for the page facts of its first,
 * and then handed a Flow that replays the recording from the beginning.
 * `auth-gate` records its way to `/scenarios/auth-gate/account`, whose armed
 * response is a 302 to `/?expired=1`, so its armed Flow run began on a page the
 * workflow never starts from. The load now precedes every Flow run, armed or
 * not. Two halves, because neither sees it alone: the destination is
 * `openScenarioStart`'s, and the choice to use it at all is the call site's.
 */
test("the Flow lane's load before every run lands on the scenario's startPath, not wherever the recording ended", async () => {
  ...
  assert.equal(reloads, 0, "the Flow's starting page is reached by address, never by reloading whatever page is open");
  ...
  assert.ok(firstLoad < flowLaneArm && flowLaneArm < flowLoad, "the Flow lane's load follows its arm point, so an armed run presents the armed rendering");
});
