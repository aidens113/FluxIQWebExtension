...
try {
  ...
  const [{ loadAllowlistedTestEnvironment }, { resolveDemoWorkspaceConfiguration, runDemoLlmCreationSettingsProbe }] = await Promise.all([
  ...
  const result = await runDemoLlmCreationSettingsProbe(config);
  process.stdout.write(JSON.stringify({ status: result.status, providerCallCount: 0 }) + "\n");
} catch {
  process.stderr.write(JSON.stringify({ status: "failed", message: "Bounded UI Flow settings probe failed" }) + "\n");
  process.exitCode = 1;
}
...
function runBuild(args) {
  return new Promise((resolve, reject) => {
    ...
    child.once("exit", code => code === 0 ? resolve() : reject(new Error("settings probe prerequisite build failed")));
  });
}