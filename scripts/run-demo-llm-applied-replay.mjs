...
try {
  const [{ loadAllowlistedTestEnvironment }, { resolveDemoWorkspaceConfiguration, runDemoLlmAppliedCreationReplayProbe }] = await Promise.all([
  ...
  const result = await runDemoLlmAppliedCreationReplayProbe(config);
  process.stdout.write(JSON.stringify(result) + "\n");
} catch {
  process.stderr.write(JSON.stringify({ status: "failed", message: "Provider-free applied creation replay failed", providerCallCount: 0 }) + "\n");
  process.exitCode = 1;
}
