...
try {
  const [{ loadAllowlistedTestEnvironment }, { resolveDemoWorkspaceConfiguration, runDemoLlmPendingCreationProbe }] = await Promise.all([
  ...
  const result = await runDemoLlmPendingCreationProbe(config);
  process.stdout.write(JSON.stringify({ status: "passed", providerCallCount: 0, ...result }) + "\n");
} catch {
  process.stderr.write(JSON.stringify({ status: "failed", message: "Provider-free pending creation inspection failed", providerCallCount: 0 }) + "\n");
  process.exitCode = 1;
}
