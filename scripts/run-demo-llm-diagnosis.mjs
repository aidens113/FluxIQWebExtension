...
try {
  ...
  const [{ loadAllowlistedTestEnvironment }, { resolveDemoWorkspaceConfiguration, runDemoLlmDiagnosis }] = await Promise.all([
  ...
  const result = await runDemoLlmDiagnosis(config);
  process.stdout.write(JSON.stringify({
    status: result.status,
    diagnosisStatus: result.diagnosisStatus,
    providerCallCount: result.providerCallCount,
    replayStatus: result.replayStatus,
    replayProviderCallCount: result.replayProviderCallCount,
    safetyPassed: result.evaluation.safetyPassed,
    leakAttestation: result.leakAttestation.status,
  }) + "\n");
} catch {
  process.stderr.write(JSON.stringify({ status: "failed", message: "Bounded UI diagnosis certification failed" }) + "\n");
  process.exitCode = 1;
}
...
function runBuild(args) {
  return new Promise((resolve, reject) => {
    ...
    child.once("exit", code => code === 0 ? resolve() : reject(new Error("diagnosis prerequisite build failed")));
  });
}
