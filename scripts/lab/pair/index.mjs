...
// decides where the two worktrees are; `campaign-environment.mjs`,
// `provider-key.mjs` and `instructions.mjs` say how to run a campaign from the
// pair; `command-line.mjs` is the entry point's work.
//
...
// what is running inside one, deciding and carrying out a move to another
// commit (`planSideMove`, `applyMove`), and running git and pnpm there.
// `scripts/task/` uses the same module, so those refusals exist once.
