...
test("a Lab child started by absolute path inside either root is found, in either slash and any case", () => {
  const processes = [
  ...
  assert.deepEqual(processesUsingRoots({ processes, roots: ROOTS, selfPid: 99 }).map((found) => found.pid), [10, 11, 12]);
});

test("a root named on its own, as an argument, does not count", () => {
  const processes = [entry(20, 1, "node scripts/lab/pair.mjs --ext-root F:/fxlab/lab-ext --core dev")];
  assert.deepEqual(processesUsingRoots({ processes, roots: ROOTS, selfPid: 99 }), []);
});

test("this process and its ancestors are the ones asking, and never count", () => {
  const processes = [
  ...
  assert.deepEqual(processesUsingRoots({ processes, roots: ROOTS, selfPid: 32 }).map((found) => found.pid), [33]);
});

test("a parent cycle in the listing does not loop", () => {
  const processes = [entry(40, 41, "node x"), entry(41, 40, "node F:/fxlab/lab-ext/y.mjs")];
  assert.deepEqual(processesUsingRoots({ processes, roots: ROOTS, selfPid: 40 }), []);
});
