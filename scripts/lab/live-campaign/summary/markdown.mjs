...
export function renderSummaryMarkdown(summary) {
  ...
  // `--replays`: whether the repair was applied (and the declared repair's verdict, where the lane judged one), then the replays with no model that met the goal.
  const repairLaneOf = ({ repairLane: lane }) => (lane ? `${lane.application ?? "unreadable"}${lane.declaredRepair ? ` (${lane.declaredRepair})` : ""}; ${lane.replaysPassed} of ${lane.replaysRequested ?? "?"} replay(s) passed, ${lane.replayProviderCalls ?? "no"} call(s)` : null);
  // What the build said it would lastingly do and who allowed it, so a run
  // that proceeded on the person's own instruction and one that stopped for a
  // permission nobody answered read differently rather than both as "failed".
  const consequencesOf = ({ consequences: c }) => {
    if (!c) return null;
    if (c.answeredBy === "not recorded") return "not recorded";
    const asked = c.permissionRequest ? `asked for ${c.permissionRequest.missing.join(", ")} and nobody answered` : null;
    const said = c.declared.length > 0 ? c.declared.join(", ") : `nothing lasting (${c.declaredNothing ?? 0} of ${c.actions ?? 0} actions)`;
    const cross = c.crossCheckVerdict === "undeclared" ? `; instruction also asks for ${c.crossCheckUndeclared.join(", ")}, declared by nothing` : "";
    return `${said}; allowed by ${c.answeredBy}${asked ? `; ${asked}` : ""}${cross}`;
  };
  ...
  if (creations.length > 0 || repairs.length === 0) {
    lines.push("", "| Task | Scenario / variant | Kind | Run | Verdict | Build | Flow created | Created Flow nodes | Declared consequences | Executed actions | Judged by | Judgement | Repair and replays | Calls | Tokens (reported) | Cost USD (reported) | Failure | Issue codes | Attempts |", "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |");
    for (const row of creations) {
      const judgedBy = row.judgeBy === "expected-dataset" ? `dataset ${row.expectedDatasetId}` : "playback goal";
      lines.push(tableRow([row.taskId, row.variantId ? `${row.scenarioId} / ${row.variantId}` : row.scenarioId, row.kind, row.runId, row.verdict, row.buildOutcome, yesNo(row.flowCreated), nodesOf(row), consequencesOf(row), row.actionTypes.join(", "), judgedBy, yesNo(row.judgement.passed), repairLaneOf(row), row.providerCalls, spent(row.reportedTokens, row), spent(row.reportedCostUsd, row), failureOf(row), row.issueCodes.join(", "), attemptsOf(row)]));
    }
  }
  if (repairs.length > 0) {
