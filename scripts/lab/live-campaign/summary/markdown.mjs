...
export function renderSummaryMarkdown(summary) {
  ...
  if (creations.length > 0 || repairs.length === 0) {
    lines.push("", "| Task | Scenario / variant | Kind | Run | Verdict | Flow created | Created Flow nodes | Executed actions | Judged by | Judgement | Repair and replays | Calls | Tokens (reported) | Cost USD (reported) | Failure | Issue codes | Attempts |", "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |");
    for (const row of creations) {
      const judgedBy = row.judgeBy === "expected-dataset" ? `dataset ${row.expectedDatasetId}` : "playback goal";
      lines.push(tableRow([row.taskId, row.variantId ? `${row.scenarioId} / ${row.variantId}` : row.scenarioId, row.kind, row.runId, row.verdict, yesNo(row.flowCreated), nodesOf(row), row.actionTypes.join(", "), judgedBy, yesNo(row.judgement.passed), repairLaneOf(row), row.providerCalls, spent(row.reportedTokens, row), spent(row.reportedCostUsd, row), failureOf(row), row.issueCodes.join(", "), attemptsOf(row)]));
    }
  }
  if (repairs.length > 0) {
