...
try {
  ...
  const failureCode = safeReason ?? (error && typeof error === "object" && error.name === "RunnerFailure" && typeof error.category === "string"
    ...
    : error && typeof error === "object" && error.name === "TimeoutError"
      ? "playwright.timeout"
      : "unknown");
  process.stderr.write(JSON.stringify({ status: "failed", message: "Provider-free evidence-guided proposal apply failed", failureCode, providerCallCount: 0, ...await demoLauncherFailureDetail(error, secretLiterals) }) + "\n");
  process.exitCode = 1;
}
