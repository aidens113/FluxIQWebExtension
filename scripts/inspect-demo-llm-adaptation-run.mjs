...
try {
  const [{ loadAllowlistedTestEnvironment }, { resolveDemoWorkspaceConfiguration, inspectLatestDemoLlmAdaptationRun }] = await Promise.all([
  ...
  const result = await inspectLatestDemoLlmAdaptationRun(resolveDemoWorkspaceConfiguration(repositoryRoot, environment));
  process.stdout.write(JSON.stringify(result) + "\n");
} catch {
  process.stderr.write(JSON.stringify({ status: "failed", reasonCode: "adaptation_state.unavailable", providerCallCount: 0 }) + "\n");
  process.exitCode = 1;
}
