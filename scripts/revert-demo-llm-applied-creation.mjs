...
try {
  const [{ loadAllowlistedTestEnvironment }, { resolveDemoWorkspaceConfiguration, runDemoLlmAppliedCreationRevertProbe }] = await Promise.all([
  ...
  const result = await runDemoLlmAppliedCreationRevertProbe(config);
  process.stdout.write(JSON.stringify(result) + "\n");
} catch {
  process.stderr.write(JSON.stringify({ status: "failed", message: "Provider-free applied creation revert failed", providerCallCount: 0 }) + "\n");
  process.exitCode = 1;
}
