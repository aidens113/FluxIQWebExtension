...
| **P10** | Recorded Flows cannot replay their honest path: an ambiguous stored target (company-website chat Close, 7 equal matches) and a step's expected state taken from a later step (local-classifieds cookie click) | downstream | — | in flight on t063 (`w2x-recording-fidelity`) |
