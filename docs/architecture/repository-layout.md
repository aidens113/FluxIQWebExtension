...
| `<package>/.lab-instances/<instance>/` | No | One concurrent Lab instance's own build output, under the package that produced it. Written only by `scripts/lab/run-lab.mjs`; removable at any time, and reclaimed by `pnpm task prune` ([Reclaiming Disposable Build Output](#reclaiming-disposable-build-output)). |
...
### Reclaiming Disposable Build Output

A label directory is cleared when its own label is used again, so a label that
keeps being used costs nothing. A label used once by a worker that has since
exited is never touched by anything, and the same is true of a Lab instance's
`.lab-instances/<instance>/`. Measured 2026-09-17, before this command existed:
162 label directories holding 1.4 GB under `apps/extension/.test-build-scratch`,
99 holding 178 MB under `domain/.test-build-scratch`, and 298 MB across four
`.lab-instances`.
...
`pnpm task prune` reclaims both:

```bash
pnpm task prune --dry-run          # decide everything, change nothing
pnpm task prune                    # anything untouched for 3 days
pnpm task prune --days 7
```

It deletes only the *children* of a scratch root, never the root, which is what
the ignore rules name and what the next build writes into. Two guards must both
pass before anything is removed, and neither is sufficient alone:

- **Age.** A directory touched within `--days` is left alone even when nothing
  is running in it, because a worker between two builds still owns its label.
  The age is the newest modification time anywhere in the tree, not the
  directory's own: a Lab instance is written through `extension/dist/` and
  `host/`, so its top-level mtime stays at whenever it was created.
- **A running process.** A directory something is working inside is left alone
  however old it looks, because an mtime says when a tree was last written, not
  whether a browser has it loaded. This is the same check that stops a worktree
  being moved out from under a run
  (`scripts/worktree/processes-using-roots.mjs`).

A Core worktree under the disposable base is removed when no worktree of this
repository sits in the same directory — the base itself for the flat layout, the
task's own `<base>/<id>/` for a Core-paired task — through the same refusals
that guard every other worktree removal: it must be below the disposable base,
be a linked worktree rather than somebody's real checkout, be clean, and have
nothing running inside it. Removing the shared Core costs the next
`--worktree` task the full Core setup (~50s) rather than the ~4s of joining an
existing one, which is the right trade when nothing is using it.

`test-runs/` is deliberately out of scope. It is ignored and disposable in the
